...
class Config:
    ...
    # DRL parameters - OPTIMIZED FOR SPEED
    HIDDEN_UNITS = [64, 64]
    LEARNING_RATE = 1e-3  # Increased for faster learning
    BATCH_SIZE = 32       # Much smaller batch size
    BUFFER_SIZE = int(1e4)  # Reduced buffer size (10k instead of 1M)
    GAMMA = 0.99
    TARGET_UPDATE_RATE = 0.005  # Slower target updates
...
class UniswapV3Environment:
    def __init__(self, data: pd.DataFrame, initial_capital: float = 1000.0, fee_tier: float = 0.003, max_episode_steps: int = 1000):
        self.data = data.copy()
        self.initial_capital = initial_capital
        self.fee_tier = fee_tier
        self.tick_spacing = Config.TICK_SPACING
        self.max_episode_steps = max_episode_steps  # Limit episode length
